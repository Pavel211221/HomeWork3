...
public class MainClass3 {
    public static void main(String[] args) {
        ...
        printArr(squareArr(5));


        System.out.println(Arrays.toString(getFillTable(10,12)));
     ...
     *  элементы одной из диагоналей можно по следующему принципу: индексы таких элементов равны, то есть [0][0], [1][1], [2][2], …, [n][n];
     * */

    public static int[][] squareArr(int n) {
        int[][] arr = new int[n][n];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (i == j || i + j == n - 1) {
    ...
    public static int[] getFillTable(int len, int initialValue) {
        int arr[] = new int[len];
        {
            for (int i = 0; i < arr.length; i++) {
                arr[i] = initialValue;
